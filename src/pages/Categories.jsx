...
import {
  ...
  createCategoriesProject,
  deleteCategoriesProject,
} from "../api/api";
...
export default function Categories() {
  ...
  const fetchProjects = async () => {
    const res = await getCategoriesProjects();
  ...
  const handleCreate = async (e) => {
    ...
    try {
      await createCategoriesProject(form);
  ...
  const handleDelete = async (id) => {
    if (confirm("Are you sure you want to delete this project?")) {
      await deleteCategoriesProject(id);
      fetchProjects();
    ...
    <div className="min-h-screen w-[100vw] bg-gray-50 px-4 py-8">
      ...
      >
        ...
        <textarea
          ...
          className="w-full text-black px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-300"
          rows={3}
        />
        <input
          type="file"
